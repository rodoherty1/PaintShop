...
class BatchFinderTest extends WordSpec with ShouldMatchers {
    
    "My Paintshop" should {
        
        "fulfill a simple order from one customer who wants 1->G" in {
            new BatchFinder(1)(List(Map(1 -> GLOSS))) should be (List(GLOSS))
        } 
        "fulfill a simple order from one customer who wants 1->M" in {
            new BatchFinder(1)(List(Map(1 -> MATTE))) should be (List(MATTE))
        } 
        
        "fulfill an order from two customers who want 1->M and 2->G" in {
            new BatchFinder(2)(List(Map(1 -> MATTE), Map(2 -> GLOSS))) should be (List(MATTE, GLOSS))
        } 
        
        "fulfill an order from one customer with 5 colours" in {
            new BatchFinder(5)(List(Map(1 -> MATTE, 3 -> GLOSS, 5-> GLOSS))) should be (List(GLOSS, GLOSS, GLOSS, GLOSS, GLOSS))
        } 
        
        "fulfill an order from two customers with 5 colours" in {
            new BatchFinder(5)(List(Map(1 -> MATTE, 3 -> GLOSS, 5-> GLOSS), Map(5->MATTE))) should be (List(GLOSS, GLOSS, GLOSS, GLOSS, MATTE))
        } 
        
        "fulfill an order from three customers with 5 colours" in {
            new BatchFinder(5)(List(Map(1 -> MATTE, 3 -> GLOSS, 5-> GLOSS), Map(2->GLOSS, 3->MATTE, 4-> GLOSS), Map(5->MATTE))) should be (List(GLOSS, GLOSS, GLOSS, GLOSS, MATTE))
        } 

        "determine if a particular batch fulfills a given order" in {
            val ps = new BatchFinder(3)
